...
- **Global scope**: `/Users/higemaru/open-llama-cli/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/higemaru/open-llama-cli/.gemini/notes/`)
...
2. Glob `/Users/higemaru/open-llama-cli/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (/Users/higemaru/open-llama-cli/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/higemaru/open-llama-cli/.gemini/notes/` — works in any directory
