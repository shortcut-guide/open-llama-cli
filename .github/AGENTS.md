...
4. **Orchestrator** (`src/orchestrator.ts`)
   - Multi-agent pipeline: Analyzer → Planner → Coder → Reviewer (with retry loop)
   - Agents are stateless functions; orchestrator controls flow

5. **Agents** (`src/agents/`)
   - Pure functions that wrap LLM calls with specific prompts
   - `analyzer.ts`: Static code analysis (exports, dependencies, functions)
   - `planner.ts`: Architecture design
...
6. **Config** (`src/config.ts`)
   - Loads from `~/.lcli.env` then `.env` (cwd)
   - Exports `Config` interface with LLM URLs, temperature, max tokens, etc.
...
- `fileProposal.ts` parses ` ```file:path/to/file` blocks from LLM responses
...
- Pipeline: `Analyzer(code)` → `Planner(task, analysis)` → `Coder(plan)` → `Reviewer(code)`
...
- **Config Externalization**: LLM endpoints are currently in code; consider full `.env` externalization
