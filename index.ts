...
let AUTO_WRITE: boolean =
  process.env.AUTO_WRITE === 'true' || process.env.AUTO_WRITE === '1';

const BASE_SYSTEM_PROMPT =
  process.env.SYSTEM_PROMPT || 'あなたは優秀なAIアシスタントです。';
...
async function saveHistory(history: Message[]): Promise<void> {
  try {
 ...
 */
async function handleFileEditProposals(
  ...
  proposals.forEach((p, i) =>
    console.log(chalk.cyan(`  [${i + 1}] ${p.filePath}`))
  );
  ...
  if (AUTO_WRITE) {
    for (const p of proposals) {
      try {
        ...
        console.error(
          chalk.red(`  ❌ 保存失敗 [${p.filePath}]: ${(e as Error).message}`)
        );
      }
    }
    return;
...
function printAutoWriteStatus(): void {
  console.log(
    AUTO_WRITE
